...
function statusIcon(file: FileStatus): { icon: string; cls: string } {
    ...
    if (s === "Added" || s === "Untracked")
        return { icon: "+", cls: "text-green-600 dark:text-green-400" };
    if (s === "Deleted")
        return { icon: "−", cls: "text-red-600 dark:text-red-400" };
    if (s === "Renamed")
        return { icon: "→", cls: "text-blue-600 dark:text-blue-400" };
    ...
    return (
        <div className="flex flex-col gap-1 p-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700 rounded text-xs">
            ...
            <div className="flex gap-1 justify-end">
                <button
                    onClick={onCancel}
                    className="px-2 py-0.5 rounded border border-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                >
                    Cancel
                </button>
                <button
...
function FileRow({
    ...
    return (
        ...
        >
            <span className={`font-mono font-bold w-3.5 shrink-0 ${cls}`}>
                {icon}
            </span>
            {isLocked && (
                <span
                    title="LFS Locked"
                    className="text-amber-500 shrink-0 text-xs"
                >
                    🔒
                </span>
            )}
...
function LockedOnlyRow({ lock, onContextMenu }: LockedOnlyRowProps) {
    return (
        ...
        >
            <span className="font-mono font-bold w-3.5 shrink-0 text-amber-500">
                🔒
            </span>
            <span className="flex-1 truncate" title={lock.filename}>
                {lock.filename}
            </span>
...
function UnstagedCtxMenu({
    ...
    useEffect(() => {
        if (menuRef.current) {
            ...
            if (y + rect.height > window.innerHeight)
                newTop = window.innerHeight - rect.height - 4;
            if (x + rect.width > window.innerWidth)
                newLeft = window.innerWidth - rect.width - 4;
    ...
    const folderCandidates = isSingle
        ? firstFile
              ...
              .reduce<string[]>((acc, part) => {
                  acc.push(
                      acc.length > 0 ? `${acc[acc.length - 1]}/${part}` : part,
                  );
                  return acc;
              }, [])
        : [];
    ...
    const btnClass =
        "w-full text-left px-4 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center justify-between";
    const dangerClass =
        "w-full text-left px-4 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-red-600 dark:text-red-400 flex items-center justify-between";
    ...
    return createPortal(
        <>
            ...
            >
                ...
                {hasUnmerged && isSingle && (
                    <>
                        ...
                        <button
                            ...
                            onClick={async () => {
                                try {
                                    await runMergetool(repoPath, [firstFile]);
                                } catch {
                                    /* ignore */
                                }
                                onClose();
                ...
                >
                    <span>
                        Stage{!isSingle ? ` (${files.length} files)` : ""}
                    </span>
                </button>
                ...
                {!hasUntracked && (
                    ...
                    >
                        <span>
                            Revert{!isSingle ? ` (${files.length} files)` : ""}
                        </span>
                        ...
                        {showGitignoreSub && (
                            ...
                            >
                                ...
                                <button
                                    className={btnClass}
                                    onClick={() =>
                                        handleAddToGitignore(firstFile)
                                    }
                                >
                                    {firstFile}
                                </button>
                    ...
                    <>
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (isModified) {
                                    alert(
                                        "Cannot unlock a modified file. Revert changes first.",
                                    );
                                    onClose();
                                    return;
                                }
    ...
    return createPortal(
        <>
            ...
            >
                ...
                >
                    <span>
                        Unstage{!isSingle ? ` (${files.length} files)` : ""}
                    </span>
                </button>
...
export function PendingTab({
    ...
    const [apiKey, setApiKey] = useState(
        () => localStorage.getItem(AI_API_KEY_STORAGE) ?? "",
    );
    ...
    const [showLocksModal, setShowLocksModal] = useState<{
        locks: LfsLock[];
        loading: boolean;
    } | null>(null);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const [result, lockableExts, sync] = await Promise.all([
                    getGitStatus(repoPath),
                    getLfsLockableExtensions(repoPath).catch(
                        () => [] as string[],
                    ),
                    getSyncStatus(repoPath),
                ]);
    ...
    const handleUnstagedClick = (
        ...
        const isUntracked =
            status?.untracked?.some((f) => f.filename === filename) ?? false;

        if (shiftKey && lastClickedUnstagedRef.current) {
            // Shift+Click: 범위 선택
            const anchorIdx = allUnstagedNames.indexOf(
                lastClickedUnstagedRef.current,
            );
    ...
    useEffect(() => {
        const onKeyDown = (e: KeyboardEvent) => {
            if (e.ctrlKey && e.key === "r") {
                ...
                e.stopPropagation();
                const {
                    selectedUnstaged: sel,
                    status: st,
                    repoPath: rp,
                } = ctrlRStateRef.current;
                if (sel.size === 0) return;
                const untrackedNames = new Set(
                    (st?.untracked ?? []).map((f) => f.filename),
                );
                const revertable = [...sel].filter(
                    (f) => !untrackedNames.has(f),
                );
                if (revertable.length === 0) return;
                buildRevertMessage(revertable, rp)
                    .then((msg) =>
                        tauriConfirm(msg, { title: "Revert", kind: "warning" }),
                    )
                    .then((ok) => {
                        if (ok)
                            gitDiscard(rp, revertable)
                                .then(() => fetchStatus())
    ...
    const handleDeleteFiles = async (files: string[]) => {
        const absolutePaths = await Promise.all(
            files.map((f) => resolve(repoPath, f)),
        );
        const ok = await tauriConfirm(
            `Delete ${files.length} file(s) permanently?\n\n${absolutePaths.join("\n")}`,
    ...
    const handleUnstagedContextMenu = (
        ...
        const hasUntracked = targetFiles.some((f) => {
            const found = [...(status?.untracked ?? [])].find(
                (u) => u.filename === f,
            );
            return !!found;
        });
        const unmergedFile =
            targetFiles.length === 1
                ? (status?.unmerged ?? []).find(
                      (u) => u.filename === targetFiles[0],
                  )
                : undefined;
        const hasUnmerged = targetFiles.some((f) => {
            const found = [...(status?.unmerged ?? [])].find(
                (u) => u.filename === f,
            );
            return !!found;
        });
    ...
    const handleConflictContextMenu = (
        e: React.MouseEvent,
        filename: string,
        file: FileStatus,
    ) => {
    ...
    const handleResolveUsing = async (
        side: "ours" | "theirs",
        files: string[],
    ) => {
        try {
    ...
    const handleAiCommitMessage = async () => {
        ...
        try {
            ...
            if (!stagedDiff.trim()) {
                setError("No staged changes to generate message from");
                return;
            }
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
                ...
                <div className="flex items-center justify-between gap-2">
                    <label className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-400 cursor-pointer select-none">
                        <input
                            ...
                            checked={amend}
                            onChange={(e) =>
                                handleAmendToggle(e.target.checked)
                            }
                    ...
                    <div className="flex items-center gap-1.5">
                        <button
                            ...
                            title={
                                apiKey
                                    ? "Generate AI commit message (Gemini)"
            ...
            {/* ── Staged files ── */}
            <div
                className="flex flex-col border-b border-gray-200 dark:border-gray-700 overflow-hidden"
                style={{ maxHeight: "40%" }}
            >
                <SectionHeader
                    label="Staged files"
                        ...
                        stagedFiles.map((file) => (
                            <div key={file.filename} className="group">
                                <FileRow
                                    file={file}
                                    isSelected={
                                        previewKey === `s:${file.filename}`
                                    }
                                    isLocked={lockedSet.has(file.filename)}
                                    onClick={(_e) =>
                                        handleFileClick(file.filename, true)
                                    }
                                    onContextMenu={(e) =>
                                        handleStagedContextMenu(
                                            e,
                                            file.filename,
                                        )
                                    }
                            ...
                            unstagedFiles.map((file) => {
                                ...
                                return (
                                    <div key={file.filename} className="group">
                                        <FileRow
                                            file={file}
                                            isSelected={selectedUnstaged.has(
                                                file.filename,
                                            )}
                                            isLocked={isLocked}
                                            onClick={(e) =>
                ...
                {hasLfsLockable && lfsLocks.length > 0 && (
                    ...
                    >
                        <SectionHeader
                            ...
                            action={{
                                label: "Show Locks",
                                onClick: handleShowLocks,
                            }}
                        />
                        <div className="overflow-y-auto min-h-[2.5rem]">
                            {lfsLocks.map((lock) => (
                                <div
                                    key={`locked:${lock.filename}`}
                                    className="group"
                                >
                                    <LockedOnlyRow
                                        lock={lock}
            ...
            {conflictCtxMenu && (
                <ConflictCtxMenu
                    ...
                    onResolveTheirs={(files) =>
                        handleResolveUsing("theirs", files)
                    }
                    onMarkResolved={handleStage}
                    onOpenMergeEditor={
                        conflictCtxMenu.singleFile
                            ? () =>
                                  onOpenMergeEditor?.(
                                      conflictCtxMenu.singleFile!,
                                  )
                    ...
                    onMergetool={
                        conflictCtxMenu.singleFile
                            ? () =>
                                  runMergetool(repoPath, [
                                      conflictCtxMenu.singleFile!.filename,
                                  ])
                            : undefined
                    }
                />
            ...
            {ctxMenu && (
                <UnstagedCtxMenu
                    ...
                    onShowInHistoryFile={onShowInHistoryFile}
                    onResolveMerge={
                        ctxMenu.unmergedFile
                            ? () => onOpenMergeEditor?.(ctxMenu.unmergedFile!)
                            : undefined
                    }
                />
            )}
                                ...
                                {showLocksModal.loading ? (
                                    ...
                                    <ul className="space-y-1">
                                        {showLocksModal.locks.map((l) => (
                                            <li
                                                key={l.lock_id || l.filename}
                                                className="text-xs font-mono text-gray-700 dark:text-gray-300 py-1 border-b border-gray-100 dark:border-gray-700 last:border-0"
                                            >
